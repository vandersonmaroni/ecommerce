 ...
 * @ngInject
 */
function requestEmailApiService($http, $location) {
  function requestGetDev (cb, route) {
    var promisse = $http.get('/api/system/uri/email');
  ...
  function requestGetProd (cb, route) {
    var uri = 'https://coletivoeducacao.com.br/api/email';
    cb($http.get(uri + route));
  }
...
factoriesModule.factory('requestEmailApiService', requestEmailApiService);