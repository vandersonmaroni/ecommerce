 ...
 * @ngInject
 */
function requestPaymentApiService($http, $location) {
  function requestGetDev (cb, route) {
    var promisse = $http.get('/api/system/uri/paymentapi');
  ...
  function requestGetProd (cb, route) {
    var uri = 'https://coletivoeducacao.com.br/api/payment';
    cb($http.get(uri + route));
  }
...
factoriesModule.factory('requestPaymentApiService', requestPaymentApiService);